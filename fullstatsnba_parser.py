...
html = requests.get(
    'https://www.basketball-reference.com/leagues/NBA_stats_per_game.html').content
...
for year in range(START_YEAR, END_YEAR):
    ...
    for conf in ['East', 'West']:
        ...
        df[['TEAM', 'Year', 'Conference Rank', 'GP']].apply(
            fill_team_rank_historical_lookup, axis=1)
    ...
    select = Select(d.find_element_by_xpath(
    ...
    df_traditional = pd.read_html(str(s_traditional))[
        0].dropna(subset=['PLAYER'])

    df_advanced = pd.read_html(str(s_advanced))[0].dropna(subset=['PLAYER'])

    df_defense = pd.read_html(str(s_defense))[0].rename(
        columns={'Player': 'PLAYER'}).dropna(subset=['PLAYER'])

    df = df_traditional.merge(
        df_advanced[['PLAYER', 'TS%', 'USG%', 'PIE']], on='PLAYER')
    df = df.merge(df_defense[['PLAYER', 'DEFWS']], on='PLAYER')

    # stitching it all together
    df['Year'] = year
    df['Avg. Pace'] = df['Year'].map(lambda x: pace_lookup[x])
    df['Team Conference Rank'] = df[['TEAM', 'Year']].apply(
        lambda row: team_rank_historical_lookup[row['Year']][row['TEAM']][0], axis=1)
    df['Team GP'] = df[['TEAM', 'Year']].apply(
        lambda row: team_rank_historical_lookup[row['Year']][row['TEAM']][1], axis=1)
    df['PLAYER'] = df['PLAYER'].map(
        lambda x: 'Ron Artest' if x == 'Metta World Peace' else x)
    df['Prior ASG Appearances'] = df[['PLAYER', 'Year']].apply(lambda row: sum(
        y <= row['Year'] for y in all_star_appearances[row['PLAYER']]), axis=1)
    df['AS Last Year?'] = df[['PLAYER', 'Year']].apply(
        was_AS_last_year, axis=1)
    df['Selected?'] = df[['PLAYER', 'Year']].apply(
        lambda row: 1 if row['Year']+1 in all_star_appearances[row['PLAYER']] else 0, axis=1)

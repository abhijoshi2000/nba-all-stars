...
for year in range(START_YEAR, END_YEAR):
    ...
    html = requests.get(
        'https://www.basketball-reference.com/allstar/NBA_{}.html'.format(year)).content
    ...
    for s in s3:
        if 'Did not play' in str(s):
            # all the injured players and their replacements
            for player in [name.get_text() for name in s.findAll('a')]:
                all_stars.add(fix_name(player))
            break
...
sorted_all_star_appearances = sorted([(player, sorted(list(
    appearances))) for player, appearances in all_star_appearances.items()], key=lambda x: -len(x[1]))
